function generateHexNumber(size = 15): string {
    const factor = Number(`1e${size}`);
    return (Math.floor(Math.random() * factor)).toString(16);
}

function padStringLeft(valor: string, tamanhoMaximo: number, replacer = ' ') {
    return `${replacer.repeat(tamanhoMaximo)}${valor}`.slice(-tamanhoMaximo);
}

function padStringRight(valor: string, tamanhoMaximo: number, replacer = ' ') {
    return `${valor}${replacer.repeat(tamanhoMaximo)}`.slice(0, tamanhoMaximo);
}

function padNumber(valor: string, tamanhoMaximo: number) {
...
function generateDate(addDays = 0): string {
...
function generateFileHeader(valores: Record<string, any>) {
    const headerArquivo = {
...
function generateFileTrailer(valores: Record<string, any>) {
...
function generateBatchHeader(valores: Record<string, any>) {
    const headerLote = {
        BancoCodigo: padStringLeft(valores.codBanco, 3, '0'),
...
function generateBatchTrailer(valores: Record<string, any>) {
    const trailerLote = {
        BancoCodigo: padStringLeft(valores.codBanco, 3, '0'),
        Lote: "0001", // Exemplo de lote de serviço
        Registro: "5",
...
function generateSegmentP(valores: Record<string, any>) {
    const segmentoP = {
        BancoCodigo: padStringLeft(valores.codBanco, 3, '0'),
        Lote: "0001", // Exemplo de lote de serviço
        Registro: "3",
...
function generateSegmentQ(valores: Record<string, any>) {
    const segmentoQ = {
        BancoCodigo: padStringLeft(valores.codBanco, 3, '0'),
        Lote: "0001", // Exemplo de lote de serviço
        Registro: "3",
...
function generateSegmentR(valores: Record<string, any>) {
    const segmentoR = {
        BancoCodigo: padStringLeft(valores.codBanco, 3, '0'),
        Lote: "0001", // Exemplo de lote de serviço
        Registro: "3",
