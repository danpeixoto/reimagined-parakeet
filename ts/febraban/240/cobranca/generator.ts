...
export function generateSegmentP(valores: Record<string, any>) {
  const segmentoP = {
    ...
    NumeroDocumento: padStringLeft(generateHexNumber() + Date.now(), 15, '0'), // Exemplo de número do documento de cobrança
    ...
    UsoEmpresaBeneficiario: padStringLeft(generateHexNumber() + Date.now(), 25, '0'), // Exemplo de identificação do título na empresa
...
export function generateSegmentQ(valores: Record<string, any>) {
  const segmentoQ = {
    ...
    NossoNumeroBancoCorrespondente: padStringLeft(generateHexNumber() + Date.now(), 20, '0'), // Exemplo de nosso número no banco correspondente
    CNAB2: "        ", // CNAB Uso Exclusivo FEBRABAN/CNAB
  };
